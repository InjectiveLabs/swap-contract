...
#[test]
fn it_reverts_if_target_quantity_is_not_multiple_of_min_quantity_tick_size() {
    let app = InjectiveTestApp::new();
    let wasm = Wasm::new(&app);
    let exchange = Exchange::new(&app);
    let bank = Bank::new(&app);
    ...
    let min_quantity = FPDecimal::must_from_str("3500.0001");
    let execute_result = wasm.execute(
        &contr_addr,
        &ExecuteMsg::SwapExactOutput {
            target_denom: ATOM.to_string(),
            target_output_quantity: min_quantity,
        },
        &[coin(12, ETH)],
        &swapper,
    );

    assert_eq!(execute_result.unwrap_err(), RunnerError::ExecuteError { msg: "failed to execute message; message index: 0: Generic error: Target quantity must be a multiple of min_quantity_tick_size: execute wasm contract failed".to_string() }, "wrong error message");
