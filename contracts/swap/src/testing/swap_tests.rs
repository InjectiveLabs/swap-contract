...
use cosmwasm_std::{
    coins, Addr, BankMsg, Binary, Coin, CosmosMsg, Reply, SubMsgResponse, SubMsgResult, Uint128,
};

use injective_cosmwasm::InjectiveMsg::CreateSpotMarketOrder;
use injective_cosmwasm::{
    get_default_subaccount_id_for_checked_address, inj_mock_env, InjectiveRoute, MarketId,
    OrderInfo, OrderType, OwnedDepsExt, SpotOrder, TEST_MARKET_ID_1, TEST_MARKET_ID_2,
};
...
fn test_swap_2_markets() {
    ...
    let response_1 = start_swap_flow(
        deps.as_mut_deps(),
        inj_mock_env(),
        info,
        "inj".to_string(),
        FPDecimal::from(2879u128),
    )
    .unwrap();
    let subaccount_id =
        get_default_subaccount_id_for_checked_address(&Addr::unchecked(TEST_CONTRACT_ADDR));
