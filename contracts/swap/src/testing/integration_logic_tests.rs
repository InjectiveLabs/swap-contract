...
use crate::testing::test_utils::{
    are_fpdecimals_approximately_equal, assert_fee_is_as_expected, create_limit_order, fund_account_with_some_inj, human_to_dec,
    init_contract_with_fee_recipient_and_get_address, init_default_signer_account, init_default_validator_account, init_rich_account,
    init_self_relaying_contract_and_get_address, launch_realistic_atom_usdt_spot_market, launch_realistic_usdt_usdc_spot_market,
    launch_realistic_weth_usdt_spot_market, must_init_account_with_funds, pause_spot_market, query_all_bank_balances, query_bank_balance,
    set_route_and_assert_success, str_coin, Decimals, OrderSide, ATOM, DEFAULT_ATOMIC_MULTIPLIER, DEFAULT_RELAYER_SHARE,
...
use injective_test_tube::{Account, Bank, Exchange, InjectiveTestApp, Module, RunnerError, RunnerResult, SigningAccount, Wasm};
use injective_testing::test_tube::exchange::launch_spot_market;
...
fn it_executes_a_swap_between_two_base_assets_with_multiple_price_levels() {
    ...
    // let spot_market_1_id = launch_spot_market_custom(&exchange, &owner, "".to_string(), ETH, USDT);
    // let spot_market_2_id = launch_spot_market_custom(&exchange, &owner, ATOM, USDT);
    ...
    let contr_addr = init_self_relaying_contract_and_get_address(&wasm, &owner, &[str_coin("100_000", USDT, Decimals::Six)]);
    ...
    assert_eq!(query_result.expected_fees.len(), 2, "Wrong number of fee denoms received");
    ...
    assert_fee_is_as_expected(&mut query_result.expected_fees, &mut expected_fees, FPDecimal::must_from_str("0.000001"));
...
fn it_executes_a_swap_between_two_base_assets_with_single_price_level() {
    ...
    let swapper = must_init_account_with_funds(&app, &[coin(3, ETH), str_coin("500_000", INJ, Decimals::Eighteen)]);
...
fn it_executes_swap_between_markets_using_different_quote_assets() {
    ...
    let spot_market_1_id = launch_realistic_weth_usdt_spot_market(&exchange, &owner);
    let spot_market_2_id = launch_realistic_atom_usdt_spot_market(&exchange, &owner);
    let spot_market_3_id = launch_realistic_usdt_usdc_spot_market(&exchange, &owner);

    let contr_addr = init_self_relaying_contract_and_get_address(
        &wasm,
        &owner,
        &[str_coin("100_000", USDC, Decimals::Six), str_coin("100_000", USDT, Decimals::Six)],
    );
    set_route_and_assert_success(
        &wasm,
        &owner,
    ...
    //USDT-USDC
    create_limit_order(&app, &trader3, &spot_market_3_id, OrderSide::Sell, 1, 100_000_000);
    ...
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::ZERO, "some of the original amount wasn't swapped");
    assert_eq!(to_balance, FPDecimal::must_from_str("2889"), "swapper did not receive expected amount");
...
fn it_reverts_swap_between_markets_using_different_quote_asset_if_one_quote_buffer_is_insufficient() {
    ...
    let contr_addr = init_self_relaying_contract_and_get_address(
        &wasm,
        &owner,
        &[str_coin("0.0001", USDC, Decimals::Six), str_coin("100_000", USDT, Decimals::Six)],
    );
    set_route_and_assert_success(
        &wasm,
        &owner,
    ...
    assert!(
        execute_result.unwrap_err().to_string().contains("Swap amount too high"),
        "incorrect query result error message"
    );
...
fn it_executes_a_buy_of_base_asset_to_receive_min_output_quantity() {
    ...
    let swapper = must_init_account_with_funds(&app, &[coin(swapper_usdt, USDT), str_coin("500_000", INJ, Decimals::Eighteen)]);
    ...
    let usdt_left_for_most_expensive_order =
        available_usdt_after_fee - (FPDecimal::from(195_000u128) * FPDecimal::from(4u128) + FPDecimal::from(192_000u128) * FPDecimal::from(3u128));
    let most_expensive_order_quantity = usdt_left_for_most_expensive_order / FPDecimal::from(201_000u128);
    let expected_quantity = most_expensive_order_quantity + (FPDecimal::from(4u128) + FPDecimal::from(3u128));

    // round to min tick
    let expected_quantity_rounded = round_to_min_tick(expected_quantity, FPDecimal::must_from_str("0.001"));
...
fn it_executes_a_swap_between_base_assets_with_external_fee_recipient() {
    ...
    let contr_addr = init_contract_with_fee_recipient_and_get_address(&wasm, &owner, &[str_coin("10_000", USDT, Decimals::Six)], &fee_recipient);
    set_route_and_assert_success(
        &wasm,
        &owner,
        &contr_addr,
    ...
    let relayer_sell_fee = buy_orders_nominal_total_value
        * FPDecimal::must_from_str(&format!("{}", DEFAULT_TAKER_FEE * DEFAULT_ATOMIC_MULTIPLIER * DEFAULT_RELAYER_SHARE));

    // calculate relayer's share of the fee based on assumptions that some of orders are matched
    let expected_nominal_buy_most_expensive_match_quantity = FPDecimal::must_from_str("488.2222155454736648");
    ...
    assert_eq!(from_balance, FPDecimal::ZERO, "some of the original amount wasn't swapped");
    assert_eq!(to_balance, FPDecimal::must_from_str("2888"), "swapper did not receive expected amount");
    ...
    assert_eq!(fee_recipient_balance.len(), 1, "wrong number of denoms in fee recipient's balances");
    assert_eq!(
...
fn it_reverts_the_swap_if_there_isnt_enough_buffer_for_buying_target_asset() {
    ...
    let contr_addr = init_self_relaying_contract_and_get_address(&wasm, &owner, &[str_coin("0.001", USDT, Decimals::Six)]);
    set_route_and_assert_success(
        &wasm,
        &owner,
        &contr_addr,
    ...
    let from_balance = query_bank_balance(&bank, ETH, swapper.address().as_str());
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::from(12u128), "source balance changes after failed swap");
...
fn it_reverts_swap_if_no_funds_were_passed() {
    ...
    let expected_error = RunnerError::ExecuteError {
        msg: "failed to execute message; message index: 0: Custom Error: \"Only one denom can be passed in funds\": execute wasm contract failed"
            .to_string(),
    };
...
fn it_reverts_swap_if_multiple_funds_were_passed() {
    ...
    assert!(
        execute_result.unwrap_err().to_string().contains("Only one denom can be passed in funds"),
        "wrong error message"
    );

    let from_balance = query_bank_balance(&bank, ETH, swapper.address().as_str());
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::from(eth_balance), "wrong ETH balance after failed swap");
    assert_eq!(to_balance, FPDecimal::from(atom_balance), "wrong ATOM balance after failed swap");
...
fn it_reverts_if_user_passes_quantities_equal_to_zero() {
    ...
    assert!(
        query_result.unwrap_err().to_string().contains("source_quantity must be positive"),
    ...
    let err = wasm
        ...
        .unwrap_err();
    assert!(
        err.to_string().contains("Output quantity must be positive!"),
        "incorrect error returned by execute"
    );

    let from_balance = query_bank_balance(&bank, ETH, swapper.address().as_str());
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::must_from_str("12"), "swap should not have occurred");
    assert_eq!(
        to_balance,
...
fn it_reverts_if_user_passes_negative_quantities() {
    ...
    assert!(execute_result.is_err(), "swap with negative minimum amount to receive did not fail");

    let from_balance = query_bank_balance(&bank, ETH, swapper.address().as_str());
    let to_balance = query_bank_balance(&bank, ATOM, swapper.address().as_str());
    assert_eq!(from_balance, FPDecimal::from(12u128), "source balance changed after failed swap");
...
fn it_reverts_if_there_arent_enough_orders_to_satisfy_min_quantity() {
    ...
    assert_eq!(
        ...
        QueryError {
            msg: "Generic error: Not enough liquidity to fulfill order: query wasm contract failed".to_string()
        },
        "wrong error message"
    );
...
fn it_reverts_if_market_is_paused() {
    ...
    assert!(
        execute_result.unwrap_err().to_string().contains("Querier contract error"),
...
fn it_reverts_if_user_doesnt_have_enough_inj_to_pay_for_gas() {
    ...
    assert_eq!(
        ...
        ExecuteError {
            msg: "spendable balance 10inj is smaller than 2500inj: insufficient funds: insufficient funds".to_string()
        },
        "wrong error returned by execute"
    );
...
fn it_allows_admin_to_withdraw_all_funds_from_contract_to_his_address() {
    ...
    let owner = must_init_account_with_funds(
        &app,
        &[eth_to_withdraw.clone(), str_coin("1", INJ, Decimals::Eighteen), usdt_to_withdraw.clone()],
    );

    let initial_contract_balance = &[eth_to_withdraw, usdt_to_withdraw];
    let contr_addr = init_self_relaying_contract_and_get_address(&wasm, &owner, initial_contract_balance);
    ...
    assert_eq!(contract_balances_after.len(), 0, "contract had some balances after withdraw");
...
fn it_doesnt_allow_non_admin_to_withdraw_anything_from_contract() {
    ...
    assert!(execute_result.is_err(), "succeeded to withdraw support funds");
    let contract_balances_after = query_all_bank_balances(&bank, &contr_addr);
    assert_eq!(
        contract_balances_after, contract_balances_before,
...
fn create_eth_buy_orders(app: &InjectiveTestApp, market_id: &str, trader1: &SigningAccount, trader2: &SigningAccount) {
...
fn create_atom_sell_orders(app: &InjectiveTestApp, market_id: &str, trader1: &SigningAccount, trader2: &SigningAccount, trader3: &SigningAccount) {
